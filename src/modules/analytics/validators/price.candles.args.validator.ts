...
import { PriceCandlesQueryArgs, PriceCandlesResolutions } from "../models/query.args";
...
export class PriceCandlesArgsValidationPipe implements PipeTransform {
    ...
    async transform(value: PriceCandlesQueryArgs, metadata: ArgumentMetadata) { 
        ...
        if (endDate.isBefore(startDate)) {
            throw new UserInputError('Invalid interval - end date before start date');
        }
        ...
        if (durationInMinutes > maxDuration) {
            throw new UserInputError('Invalid interval - selected time window is too large');
        }
