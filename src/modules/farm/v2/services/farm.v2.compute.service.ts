...
} from "../../../../submodules/weekly-rewards-splitting/services/progress.compute.service";
...
export class FarmComputeServiceV2 extends Mixin(FarmComputeService, WeeklyRewardsSplittingComputeService) {
    ...
    async computeFarmBaseAPR(farmAddress: string): Promise<string> {
        ...
        const apr = this.computeBaseRewards(new BigNumber(totalRewardsPerYearUSD), boostedYieldsRewardsPercenatage)
    ...
    async computeFarmRewardsForPosition(
        ...
        const [totalFarmRewards, boostedYieldsRewardsPercenatage] = await Promise.all([
            super.computeFarmRewardsForPosition(positon, rewardPerShare),
            this.farmGetter.getBoostedYieldsRewardsPercenatage(positon.farmAddress)
        ]);
        return this.computeBaseRewards(totalFarmRewards, boostedYieldsRewardsPercenatage);
    }

    computeBaseRewards(totalFarmRewards: BigNumber, boostedYieldsRewardsPercenatage: number): BigNumber {
        const boostedYieldsRewardsPercenatageBig = new BigNumber(boostedYieldsRewardsPercenatage);
    ...
    async computeUserRewardsForWeek(
        ...
        const boostedYieldsFactors = await this.farmGetter.getBoostedYieldsFactors(scAddress);

        const userHasMinEnergy = new BigNumber(energyAmount).isGreaterThan(boostedYieldsFactors.minEnergyAmount);
        if (!userHasMinEnergy) {
            return payments;
        }

        const userMinFarmAmount = new BigNumber(liquidity).isGreaterThan(boostedYieldsFactors.minFarmAmount);
        ...
        const [
            ...
            this.weeklyRewardsSplittingGetter.totalEnergyForWeek(scAddress, week),
        ...
        const userRewardsForWeek = new BigNumber(boostedYieldsFactors.userRewardsBase)
        ...
        for (const weeklyRewards of totalRewards) {
            ...
            const constantsBase = new BigNumber(boostedYieldsFactors.userRewardsEnergy)
            ...
            const paymentAmount = boostedRewardAmount.comparedTo(userRewardsForWeek) < 1 ? boostedRewardAmount : userRewardsForWeek;
