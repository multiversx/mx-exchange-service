...
export const filteredPairsPipeline = (
    ...
    if (filters.minVolume || sortField === PairSortableFields.VOLUME_24) {
        addFields.volumeUSD24hNum = convertToDecimalExpr('volumeUSD24h');
    }

    if (filters.minLockedValueUSD || sortField === PairSortableFields.TVL) {
        addFields.lockedValueUSDNum = convertToDecimalExpr('lockedValueUSD');
    }

    if (sortField === PairSortableFields.FEES_24) {
        addFields.feesUSD24hNum = convertToDecimalExpr('feesUSD24h');
    }
...
const convertFilterToQuery = (
    ...
    if (filter.searchToken && filter.searchToken.trim().length >= 1) {
        ...
        query.$or = [
            { firstTokenId: { $regex: regex, $options: 'i' } },
            { secondTokenId: { $regex: regex, $options: 'i' } },
        ];
    }
