...
export class PairResolver {
    ...
    async firstToken(parent: PairModel): Promise<EsdtToken> {
        return this.pairAbiLoader.tokenLoader.load(parent.firstTokenId);
        // return this.pairAbiLoader.firstTokenLoader.load(parent.address);
    }

    @ResolveField()
    async secondToken(parent: PairModel): Promise<EsdtToken> {
        return this.pairAbiLoader.tokenLoader.load(parent.secondTokenId);
    ...
    async liquidityPoolToken(parent: PairModel): Promise<EsdtToken> {
        if (parent.liquidityPoolTokenId) {
            return this.pairAbiLoader.tokenLoader.load(
                parent.liquidityPoolTokenId,
            );
        }
        return undefined;
    ...
    async rewardTokens(parent: PairModel): Promise<PairRewardTokensModel> {
        const poolRewards = await Promise.all([
            this.pairAbiLoader.tokenLoader.load(parent.firstTokenId),
            this.pairAbiLoader.tokenLoader.load(parent.secondTokenId),
        ]);
