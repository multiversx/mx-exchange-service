...
describe('WeekTimekeepingService', () => {
    ...
    it(
        'getWeeklyTimekeeping' + 'getCurrentWeek throws error should error',
        async () => {
            const service = await createService({
            ...
            await expect(
                service.getWeeklyTimekeeping(dummyScAddress),
            ).rejects.toThrowError(expectedErr);
        },
    );
    it('getWeeklyTimekeeping' + 'should work', async () => {
        ...
        expect(await service.getWeeklyTimekeeping(expectedScAddress)).toEqual(
    ...
    it('getWeekForEpoch' + 'should work', async () => {
        const expectedScAddress = 'expectedErd';
        const expectedEpoch = 10;
        const service = await createService({});
        expect(
            await service.getWeekForEpoch(expectedScAddress, expectedEpoch),
        ).toEqual(
