    ...
    it('advanceWeek' +
        ...
        const service = await createService({
            depleteUserEnergy:(
                ...
                energyEntry.amount = new BigNumber(energyEntry.amount).minus(5).toFixed()
                return energyEntry;
            },
        })
        ...
        const progress = createMockProgress()
        const returnedProgress = await service.advanceWeek(progress, undefined, 7)
        expect(returnedProgress.week)
            .toEqual(currentWeek + 1);
        expect(returnedProgress.energy.lastUpdateEpoch)
            .toEqual(currentEpoch)
        expect(returnedProgress.energy.amount)
            .toEqual("95");
    });
    it('advanceWeek' +
        'given nextWeekEnergy should replace current', async () => {
        ...
        const returnedProgress = await service.advanceWeek(progress, expectedEnergy, 7)
        expect(returnedProgress.energy)
            .toEqual(expectedEnergy);
    });
});

export function createMockProgress(currentWeek = 10, currentEpoch = 70): ClaimProgress {
