...
export class ESOperationsService {
    ...
    async getRouterMultiSwapTxs(
        startTimestamp: number,
        endTimestamp: number,
    ): Promise<Operation[]> {
        const pagination = new ElasticPagination();
        pagination.size = 2000;
        ...
        elasticQuery.filter = [
            ...
            new TermFilter('receiver', scAddress.routerAddress),
            new TermFilter('function', 'multiPairSwap'),
            new TermFilter('operation', 'ESDTTransfer'),
    ...
    async getTokenOutputForMultiSwap(
        originalTxHash: string,
        sender: string,
        receiver: string,
    ): Promise<EsdtTokenPayment | undefined> {
        const pagination = new ElasticPagination();
        pagination.size = 1;
        const elasticQuery: ElasticQuery = new ElasticQuery().withPagination(
            pagination,
        ...
        elasticQuery.filter = [
            new TermFilter('originalTxHash', originalTxHash),
            new TermFilter('sender', sender),
            new TermFilter('receiver', receiver),
            new TermFilter('operation', 'ESDTTransfer'),
        ];
        ...
        const tx = operations.at(0);
        return new EsdtTokenPayment({
            tokenID: tx.tokens[0],
            amount: tx.esdtValues[0],
        });
    }
