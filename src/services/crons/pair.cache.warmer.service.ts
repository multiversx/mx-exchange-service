...
import { TimeSeriesQueryService } from '../time-series/time-series.query.service';
...
export class PairCacheWarmerService {
    ...
    async cachePairsAnalytics(): Promise<void> {
        ...
        for (const pairAddress of pairsAddresses) {
            const firstTokenVolume24h = await this.timeSeriesQuery.getAggregatedValue({
            ...
            delay(1000);
            const secondTokenVolume24h = await this.timeSeriesQuery.getAggregatedValue(
                {
                    table: awsConfig.timestream.tableName,
                    series: pairAddress,
            ...
            delay(1000);
            const volumeUSD24h = await this.timeSeriesQuery.getAggregatedValue({
                table: awsConfig.timestream.tableName,
                series: pairAddress,
                metric: 'volumeUSD',
                time,
            ...
            delay(1000);
            const feesUSD24h = await this.timeSeriesQuery.getAggregatedValue({
                table: awsConfig.timestream.tableName,
                series: pairAddress,
                metric: 'feesUSD',
                time,
